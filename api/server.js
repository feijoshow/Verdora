...
app.get('/health', (_req, res) => {
  ok(res, {
    ...
    zaiModel: ZAI_VISION_MODEL,
  });
});

/** POST /api/v1/chat/message — Grok proxy */
app.post('/api/v1/chat/message', async (req, res) => {
  if (!XAI_API_KEY) {
    return fail(res, 503, 'XAI_API_KEY is not configured on the API server');
  }
...
app.listen(PORT, () => {
  ...
  console.log(`  Z.ai: ${ZAI_API_KEY ? `configured (${ZAI_VISION_MODEL})` : 'missing ZAI_API_KEY'}`);
